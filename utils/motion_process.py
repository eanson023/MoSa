...
def guo2spatio_representation(data):
    ...
    # get velocity for each joint
    start_indx = 1 + 2 + 1 + (joints_num - 1) * 9
    end_indx = start_indx + joints_num * 3
    velocity_params = data[..., start_indx:end_indx]
    velocity_params = velocity_params.view(bs, seq_len, joints_num, 3)

    rot_pos = torch.cat([cont6d_params, pos_params, velocity_params], dim=-1) 

     # (˙ra, r˙x, r˙z, ry)
    start_indx = 0
    end_indx = 1 + 2 + 1
    r_params = data[..., start_indx:end_indx]

    # get foot contact for each joint
    start_indx = 1 + 2 + 1 + (joints_num - 1) * 9 + joints_num * 3
    foot_params = data[..., start_indx:]

    joint_first_params = torch.cat([r_params, foot_params], dim=-1)
    joint_first_params = F.pad(joint_first_params, (0, rot_pos.size(-1)-joint_first_params.size(-1)))

    rot_pos = torch.cat([joint_first_params.unsqueeze(2),rot_pos], dim=2) #  # bs x seqlen x (num_joints+1) x 12

    return rot_pos


def spatio2guo_representation(rot_pos):
    bs, seq_len, joint_num, dims = rot_pos.shape
    joint_num = joint_num-1
    joint_first_params = rot_pos[:, :, 0]
    rot_pos = rot_pos[:, :, 1:]

    start_indx = 0
    end_indx = 1 + 2 + 1
    r_params = joint_first_params[..., start_indx:end_indx]

    # get foot contact for each joint
    start_indx = 1 + 2 + 1
    end_indx = start_indx + 4
    foot_params = joint_first_params[..., start_indx:end_indx]

    cont6d_params = rot_pos[..., :6].reshape(bs, seq_len, -1)
    cont6d_params = cont6d_params[..., 6:]
    pos_params = rot_pos[..., 6:9].reshape(bs, seq_len, -1)
    pos_params = pos_params[..., 3:]
    velocity_params = rot_pos[..., 9:].reshape(bs, seq_len, -1)

    guo_repr = torch.cat([r_params, pos_params, cont6d_params, velocity_params, foot_params], dim=-1)
