...
def evaluation_vqvae_plus_mpjpe_cbusage(val_loader, net, repeat_id, eval_wrapper, num_joint):
    ...
    for batch in val_loader:
        ...
        pred_pose_eval, *_, indices = net.reconstruction(motion, m_length, caption)
...
def evaluation_transformer(out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div,
    ...
    for batch in val_loader:
        ...
        pred_motions = vq_model.generation(pred_quantized, m_length, clip_text)
...
@torch.no_grad()
def evaluation_transformer_refine(out_dir, val_loader, refiner, trans, vq_model, writer, ep, best_fid, best_div,
                           best_top1, best_top2, best_top3, best_matching, eval_wrapper, plot_func,
                           save_ckpt=False, save_anim=False):
    ...
    for batch in val_loader:
        ...
        # (b, seqlen)
        pred_quantized, emb, quant = trans.generate(clip_text, m_length, cond_scale, vq_model, temperature=1, return_last=True)
        """ 
        key diff
        """
        # pred_motions = vq_model.forward_decoder(pred_quantized)
        bs, d, l = pred_quantized.shape
        pred_residual = refiner.sample(quant.reshape(bs * l, -1), 1, cond_scale)
        pred_residual = pred_residual.reshape(bs, -1, l)
        pred_motions = vq_model.forward_decoder(quant+pred_residual)
