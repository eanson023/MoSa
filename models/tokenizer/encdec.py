...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        B, _, T = qkv.shape  # should be B,3C,H,W
        C = self.C
        q, k, v = qkv.reshape(B, 3, C, T).unbind(1)
        
        # compute attention
        q = q.view(B, C, T).contiguous()
        q = q.permute(0, 2, 1).contiguous()     # B,HW,C
        k = k.view(B, C, T).contiguous()    # B,C,HW
        w = torch.bmm(q, k).mul_(self.w_ratio)  # B,HW,HW    w[B,i,j]=sum_c q[B,i,C]k[B,C,j]
        w = F.softmax(w, dim=2)
        
        # attend to values
        v = v.view(B, C, T).contiguous()
    ...
    def __init__(
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                down.downsample = Downsample1x(block_in)
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            up.block = block
            up.attn = attn
            up.upsample = Upsample1x(block_in)
...
if __name__ == "__main__":
    ...
    encoder = Encoder(ch=128, ch_mult=(1, 2, 4), num_res_blocks=1, dropout=0.0, in_channels=263, z_channels=512)
    decoder = Decoder(ch=128, ch_mult=(1, 2, 4), num_res_blocks=1, dropout=0.0, in_channels=263, z_channels=512)
    ...
    input_data = torch.randn((64, 263, 192))
    with torch.no_grad():
